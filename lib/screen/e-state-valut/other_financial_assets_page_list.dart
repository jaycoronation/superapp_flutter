...
import 'add_other_financial_assets_page.dart';

class OtherFinancialAssetsListPage extends StatefulWidget {
  const OtherFinancialAssetsListPage({Key? key}) : super(key: key);

  @override
  _OtherFinancialAssetsListPageState createState() => _OtherFinancialAssetsListPageState();
}

class _OtherFinancialAssetsListPageState extends BaseState<OtherFinancialAssetsListPage> {
  ...
  Future<void> _redirectAdd(InvestmentTrustAccounts listData, bool isFor) async {
    final result = await Navigator.push(
      context,
      MaterialPageRoute(builder: (context) => AddOtherFinancialAssetsPage(listData, isFor)),
    );
    print("result ===== $result");
    if (result == "success") {
      setState(() {
  ...
  _getApiData() async {
    ...
    final url = Uri.parse(API_URL_VAULT + otherFinancialAssetsList);
    ...
    var dataResponse = OtherFinancialAssetsResponse.fromJson(user);

    if (statusCode == 200 && dataResponse.success == 1)
    {
      try {
        if (dataResponse.investmentTrustAccounts != null) {
          listData = dataResponse.investmentTrustAccounts!;
          setState(() {
            _isLoading = false;
          });
        }
  ...
  void deleteListData(InvestmentTrustAccounts data, int index) {
  ...
  void castStatefulWidget() {
    widget is OtherFinancialAssetsListPage;
  }

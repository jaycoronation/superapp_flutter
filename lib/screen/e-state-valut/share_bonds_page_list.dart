...
import '../../model/e-state-vault/SharesBondsResponse.dart';
import '../../utils/app_utils.dart';
import '../../utils/my_toolbar.dart';
import '../../widget/loading.dart';
import 'add_share_bonds.dart';

class ShareBondsListPage extends StatefulWidget {
  const ShareBondsListPage({Key? key}) : super(key: key);

  @override
  _ShareBondsListPageState createState() => _ShareBondsListPageState();
}

class _ShareBondsListPageState extends BaseState<ShareBondsListPage> {
  bool _isLoading = false;
  List<SharesBonds> listData = List<SharesBonds>.empty(growable: true);
  ...
  Future<void> _redirectAdd(SharesBonds listData, bool isFor) async {
    final result = await Navigator.push(
      context,
      MaterialPageRoute(builder: (context) => AddSharesBondsPage(listData, isFor)),
    );
    print("result ===== $result");
    if (result == "success") {
      setState(() {
  ...
  _getApiData() async {
    ...
    final url = Uri.parse(API_URL_VAULT + sharesBondsList);
    ...
    var dataResponse = SharesBondsResponse.fromJson(user);

    if (statusCode == 200 && dataResponse.success == 1)
    {
      try {
        if (dataResponse.sharesBonds != null) {
          listData = dataResponse.sharesBonds!;
          setState(() {
            _isLoading = false;
          });
        }
  ...
  void deleteListData(SharesBonds data, int index) {
  ...
  void deleteData(int index) async {
    ...
    final url = Uri.parse(API_URL_VAULT + deleteSharesBonds);

    Map<String, String> jsonBody = {
      'shares_bonds_id': listData[index].sharesBondsId.toString(),
    };

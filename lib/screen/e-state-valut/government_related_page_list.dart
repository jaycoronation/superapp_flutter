...
import '../../model/e-state-vault/GovernmentRelatedResponse.dart';
import '../../utils/app_utils.dart';
import '../../utils/my_toolbar.dart';
import '../../widget/loading.dart';
import 'add_government_related.dart';

class GovernmentRelatedListPage extends StatefulWidget {
  const GovernmentRelatedListPage({Key? key}) : super(key: key);

  @override
  _GovernmentRelatedListPageState createState() => _GovernmentRelatedListPageState();
}

class _GovernmentRelatedListPageState extends BaseState<GovernmentRelatedListPage> {
  bool _isLoading = false;
  List<GovernmentRelatedDetails> listData = List<GovernmentRelatedDetails>.empty(growable: true);
  ...
  Future<void> _redirectAdd(GovernmentRelatedDetails listData, bool isFor) async {
    final result = await Navigator.push(
      context,
      MaterialPageRoute(builder: (context) => AddGovernmentRelatedPage(listData, isFor)),
    );
    print("result ===== $result");
    if (result == "success") {
      setState(() {
  ...
  _getApiData() async {
    ...
    final url = Uri.parse(API_URL_VAULT + governmentRelatedList);
    ...
    var dataResponse = GovernmentRelatedResponse.fromJson(user);

    if (statusCode == 200 && dataResponse.success == 1)
    {
      try {
        if (dataResponse.governmentRelatedDetails != null) {
          listData = dataResponse.governmentRelatedDetails!;
          setState(() {
            _isLoading = false;
          });
        }
  ...
  void deleteListData(GovernmentRelatedDetails data, int index) {
  ...
  void deleteData(int index) async {
    ...
    final url = Uri.parse(API_URL_VAULT + deleteGovernmentRelated);

    Map<String, String> jsonBody = {
      'government_related_id': listData[index].governmentRelatedId.toString(),
    };
  ...
  void castStatefulWidget() {
    widget is GovernmentRelatedListPage;
  }

...
class AddGovernmentRelatedPage extends StatefulWidget {
  GovernmentRelatedDetails data;
  final bool isForEdit;

  AddGovernmentRelatedPage(this.data, this.isForEdit, {Key? key}) : super(key: key);

  @override
  BaseState<AddGovernmentRelatedPage> createState() => _AddGovernmentRelatedPageState();
}

class _AddGovernmentRelatedPageState extends BaseState<AddGovernmentRelatedPage> {
  bool _isLoading = false;
  bool isForEdit = false;
  bool isSubmitClick = false;
  var itemData = GovernmentRelatedDetails();
  List<GovernmentRelatedDetails> listData = List<GovernmentRelatedDetails>.empty(growable: true);

  @override
  void initState() {
    super.initState();
    isForEdit = (widget as AddGovernmentRelatedPage).isForEdit;
    if (isForEdit)
    {
      itemData = (widget as AddGovernmentRelatedPage).data;
    ...
    {
      addMoreViews(GovernmentRelatedDetails());
    }
  }
  ...
  Widget build(BuildContext context) {
    return Scaffold(
        ...
        appBar: AppBar(
          ...
          title: MyToolBar(pageName: isForEdit ? "Update Gov. Related" : "Add Gov. Related"),
          centerTitle: false,
          elevation: 0,
          backgroundColor: appBg,
        ),
  ...
  void addMoreViews(GovernmentRelatedDetails data) {
    setState(() {
  ...
  _saveDataCall(String data) async {
    ...
    final url = Uri.parse(API_URL_VAULT + saveGovernmentRelated);
  ...
  void castStatefulWidget() {
    widget is AddGovernmentRelatedPage;
  }
}

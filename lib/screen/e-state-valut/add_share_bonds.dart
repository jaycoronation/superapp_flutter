...
class AddSharesBondsPage extends StatefulWidget {
  SharesBonds data;
  final bool isForEdit;

  AddSharesBondsPage(this.data, this.isForEdit, {Key? key}) : super(key: key);

  @override
  BaseState<AddSharesBondsPage> createState() => _AddSharesBondsPageState();
}

class _AddSharesBondsPageState extends BaseState<AddSharesBondsPage> {
  bool _isLoading = false;
  bool isForEdit = false;
  bool isSubmitClick = false;
  var itemData = SharesBonds();
  ...
  {
    super.initState();
    isForEdit = (widget as AddSharesBondsPage).isForEdit;
    if (isForEdit)
    {
      itemData = (widget as AddSharesBondsPage).data;
  ...
  Widget build(BuildContext context)
  {
    return Scaffold(
        ...
        appBar: AppBar(
          ...
          title: MyToolBar(pageName: isForEdit ? "Update Share Bonds" : "Add Share Bonds"),
          centerTitle: false,
          elevation: 0,
          backgroundColor: appBg,
        ),
  ...
  void addMoreViews(SharesBonds data)
  {
    setState(() {
  ...
  _saveDataCall(String data) async
  {
    ...
    final url = Uri.parse(API_URL_VAULT + saveSharesBonds);

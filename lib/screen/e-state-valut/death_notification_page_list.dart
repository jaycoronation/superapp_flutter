...
class _DeathNotificationPageState extends BaseState<DeathNotificationPage> {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      appBar: AppBar(
        ...
        title: getTitle("Death Notifications"),
        centerTitle: true,
        elevation: 0,
        backgroundColor: appBg,
      ),
  ...
  Future<void> _redirectAdd(Notifications listData, bool isFor) async {
    final result = await Navigator.push(
      context,
      MaterialPageRoute(builder: (context) => AddDeathNotificationPage(listData, isFor)),
    );
    print("result ===== $result");
    if (result == "success") {
      setState(() {
  ...
  void deleteListData(Notifications data, int index) {
  ...
  void deleteData(int index) async {
    ...
    final url = Uri.parse(API_URL_VAULT + deleteDeathNotification);

    Map<String, String> jsonBody = {
      'notification_id': listData[index].notificationId.toString(),
    };
  ...
  void deleteModule() async {
    ...
    Map<String, String> jsonBody = {
      'module':"delete-death_notification",

...
import 'add_former_spouse.dart';

class FormerSpouseListPage extends StatefulWidget {
  const FormerSpouseListPage({Key? key}) : super(key: key);

  @override
  _FormerSpouseListPageState createState() => _FormerSpouseListPageState();
}

class _FormerSpouseListPageState extends BaseState<FormerSpouseListPage> {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      appBar: AppBar(
        ...
        title: const MyToolBar(pageName: "Former Spouse/ Children from previous marriage"),
        centerTitle: false,
        elevation: 0,
        backgroundColor: appBg,
      ),
                  ...
                  children: [
                    ...
                    Row(
                      children: [
                        ...
                        Expanded(flex: 4, child: Text(
                          checkValidString(listData[index].relationship),
                          style: const TextStyle(color: black, fontSize: 16, fontWeight: FontWeight.w500),
                        )),
                      ],
                    ),
                    ...
                    Row(
                      children: [
                        ...
                        Expanded(flex: 4, child: Text(
                          checkValidString(listData[index].obligation),
                          style: const TextStyle(color: black, fontSize: 16, fontWeight: FontWeight.w500),
                        )),
                      ],
                    ),
  ...
  Future<void> _redirectAdd(FormerSpouseOfFormerMarriages listData, bool isFor) async {
    final result = await Navigator.push(
      context,
      MaterialPageRoute(builder: (context) => AddFormerSpousePage(listData, isFor)),
    );
    print("result ===== $result");
    if (result == "success") {
      setState(() {
  ...
  _getApiData() async {
    ...
    final url = Uri.parse(API_URL_VAULT + formerSpouseList);
    ...
    var dataResponse = FormerSpouseResponse.fromJson(user);

    if (statusCode == 200 && dataResponse.success == 1)
    {
      try {
        if (dataResponse.formerSpouseOfFormerMarriages != null) {
          listData = dataResponse.formerSpouseOfFormerMarriages!;
          setState(() {
            _isLoading = false;
          });
        }
  ...
  void deleteListData(FormerSpouseOfFormerMarriages data, int index) {
  ...
  void deleteData(int index) async {
    ...
    final url = Uri.parse(API_URL_VAULT + deleteFormerSpouseList);

    Map<String, String> jsonBody = {
      'former_spouse_id': listData[index].formerSpouseId.toString(),
    };
  ...
  void castStatefulWidget() {
    widget is FormerSpouseListPage;
  }
